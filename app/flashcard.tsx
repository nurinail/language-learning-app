...
export default function FlashcardScreen() {
    ...
    const load = async () => {
      try {
        ...
        const results = await Promise.all(
          data.map(async (word: string, i: number) => {
            try {
  ...
  const saveResults = async (newKnown: string[], newUnknown: string[]) => {
    let existingKnown = await getData("knownWords");
    let existingUnknown = await getData("unknownWords");

    // 🧠 normalize EVERYTHING to string[]
    existingKnown = Array.isArray(existingKnown)
      ? existingKnown.map(String)
      : [];

    existingUnknown = Array.isArray(existingUnknown)
      ? existingUnknown.map(String)
      : [];

    const mergedKnown = Array.from(
      new Set([...existingKnown, ...newKnown])
    );

    const mergedUnknown = Array.from(
      new Set([...existingUnknown, ...newUnknown])
    );
